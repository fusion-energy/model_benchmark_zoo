...
common_geometry_object = ThreeTouchingCuboids(width=5)
# just writing a CAD step file for visulisation
common_geometry_object.export_stp_file("three_touching_cuboids.stp")
...
# Create a DT point source in the void cuboid (bottom-right, center at width/2, -width/2, 0)
my_source = openmc.IndependentSource()
my_source.space = openmc.stats.Point((2.5, -2.5, 0))
...
common_geometry_object.export_h5m_file_with_cad_to_dagmc(
    filename='three_touching_cuboids.h5m',
    material_tags=['1', '2', '3'],
    min_mesh_size=0.01,
    max_mesh_size=0.5
)
dag_model = common_geometry_object.dagmc_model(
    h5m_filename='three_touching_cuboids.h5m',
