from model_benchmark_zoo import NurbsTruncatedCone
import openmc
...
common_geometry_object = NurbsTruncatedCone(height=10, bottom_radius=5, top_radius=2)
# just writing a CAD step file for visulisation
common_geometry_object.export_stp_file("nurbs_truncated_cone.stp")
...
# Create a DT point source at the centre of the frustum
...
common_geometry_object.export_h5m_file_with_cad_to_dagmc(
    filename='nurbs_truncated_cone.h5m',
    material_tags=['1'],
    min_mesh_size=0.01,
    max_mesh_size=0.5
)
dag_model = common_geometry_object.dagmc_model(
    h5m_filename='nurbs_truncated_cone.h5m',
    materials=[mat1]
)
