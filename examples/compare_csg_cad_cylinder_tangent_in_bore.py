from model_benchmark_zoo import CylinderTangentInBore
import openmc
...
common_geometry_object = CylinderTangentInBore(inner_radius=6, outer_radius=8, height=10, rod_radius=2.5)
# just writing a CAD step file for visulisation
common_geometry_object.export_stp_file("cylinder_tangent_in_bore.stp")
...
# Create a DT point source at the centre of the rod
my_source = openmc.IndependentSource()
my_source.space = openmc.stats.Point((3.5, 0, 0))
...
common_geometry_object.export_h5m_file_with_cad_to_dagmc(
    filename='cylinder_tangent_in_bore.h5m',
    material_tags=['1', '2'],
    min_mesh_size=0.01,
    max_mesh_size=0.5
)
dag_model = common_geometry_object.dagmc_model(
    h5m_filename='cylinder_tangent_in_bore.h5m',
    materials=[mat1, mat2]
)
