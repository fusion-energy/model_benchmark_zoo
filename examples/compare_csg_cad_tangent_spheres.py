from model_benchmark_zoo import TangentSpheres
import openmc
...
common_geometry_object = TangentSpheres(first_radius=5, second_radius=4)
# just writing a CAD step file for visulisation
common_geometry_object.export_stp_file("tangent_spheres.stp")
...
# Create a DT point source just inside the larger sphere, close to the point of contact
my_source = openmc.IndependentSource()
my_source.space = openmc.stats.Point((-0.5, 0, 0))
...
common_geometry_object.export_h5m_file_with_cad_to_dagmc(
    filename='tangent_spheres.h5m',
    material_tags=['1', '2'],
    min_mesh_size=0.01,
    max_mesh_size=0.5
)
dag_model = common_geometry_object.dagmc_model(
    h5m_filename='tangent_spheres.h5m',
    materials=[mat1, mat2]
)
