from model_benchmark_zoo import LargePlanarFace
import openmc
...
common_geometry_object = LargePlanarFace(span=50, thickness=5)
# just writing a CAD step file for visulisation
common_geometry_object.export_stp_file("large_planar_face.stp")
...
# Create a DT point source at the centre of the plate
...
common_geometry_object.export_h5m_file_with_cad_to_dagmc(
    filename='large_planar_face.h5m',
    material_tags=['1'],
    min_mesh_size=0.01,
    max_mesh_size=0.5
)
dag_model = common_geometry_object.dagmc_model(
    h5m_filename='large_planar_face.h5m',
    materials=[mat1]
)
