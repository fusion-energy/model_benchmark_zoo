...
common_geometry_object = Hemisphere(radius=10)
# just writing a CAD step file for visulisation
common_geometry_object.export_stp_file("hemisphere.stp")
...
my_source.space = openmc.stats.Point((0, 0, 3))
...
common_geometry_object.export_h5m_file_with_cad_to_dagmc(
    filename='hemisphere.h5m',
    material_tags=['1'],
    min_mesh_size=0.01,
    max_mesh_size=0.5
)
dag_model = common_geometry_object.dagmc_model(
    h5m_filename='hemisphere.h5m',
    materials=[mat1]
)
