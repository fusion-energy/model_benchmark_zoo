from model_benchmark_zoo import CrossShaped
import openmc
...
common_geometry_object = CrossShaped(length=10, width=10, leg_thickness=3, height=5)
# just writing a CAD step file for visulisation
common_geometry_object.export_stp_file("cross_shaped.stp")
...
# Create a DT point source at the centre where the two bars cross
...
common_geometry_object.export_h5m_file_with_cad_to_dagmc(
    filename='cross_shaped.h5m',
    material_tags=['1'],
    min_mesh_size=0.01,
    max_mesh_size=0.5
)
dag_model = common_geometry_object.dagmc_model(
    h5m_filename='cross_shaped.h5m',
    materials=[mat1]
)
