...
def test_compare(kwargs):
    ...
    common_geometry_object = EllipticCylinder(a=8, b=5, height=15)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("elliptic_cylinder.stp")
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='elliptic_cylinder.h5m',
        material_tags=['1'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='elliptic_cylinder.h5m',
        materials=[mat1]
    )
