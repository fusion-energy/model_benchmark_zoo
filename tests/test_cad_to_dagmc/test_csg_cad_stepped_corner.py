from model_benchmark_zoo import SteppedCorner
...
def test_compare(kwargs):
    ...
    common_geometry_object = SteppedCorner(width=10, corner_width=5)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("stepped_corner.stp")
    ...
    # Create a DT point source in the octant diagonally opposite the removed one
    my_source = openmc.IndependentSource()
    my_source.space = openmc.stats.Point((-2.5, -2.5, -2.5))
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='stepped_corner.h5m',
        material_tags=['1'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='stepped_corner.h5m',
        materials=[mat1]
    )
