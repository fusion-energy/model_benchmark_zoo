from model_benchmark_zoo import ThinWalledBox
...
def test_compare(kwargs):
    ...
    common_geometry_object = ThinWalledBox(width=10, wall_thickness=0.2)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("thin_walled_box.stp")
    ...
    # Create a DT point source inside the wall, halfway through its thickness
    my_source = openmc.IndependentSource()
    my_source.space = openmc.stats.Point((0, 0, 4.9))
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='thin_walled_box.h5m',
        material_tags=['1'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='thin_walled_box.h5m',
        materials=[mat1]
    )
