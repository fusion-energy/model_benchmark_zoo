...
def test_compare(kwargs):
    ...
    common_geometry_object = AnnularSector(inner_radius=3, outer_radius=6, height=10, angle=90)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("annular_sector.stp")
    ...
    # Create a DT point source at midpoint of sector (45 degrees)
    my_source = openmc.IndependentSource()
    my_source.space = openmc.stats.Point((3.18, 3.18, 0))
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='annular_sector.h5m',
        material_tags=['1'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='annular_sector.h5m',
        materials=[mat1]
    )
