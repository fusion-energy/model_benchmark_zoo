from model_benchmark_zoo import NotchedBox
...
def test_compare(kwargs):
    ...
    common_geometry_object = NotchedBox(width=10, depth=10, height=10, notch_width=4, notch_depth=4, notch_height=4)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("notched_box.stp")
    ...
    # Create a DT point source in the solid below the notch
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='notched_box.h5m',
        material_tags=['1'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='notched_box.h5m',
        materials=[mat1]
    )
