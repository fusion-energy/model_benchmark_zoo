...
def test_compare(kwargs):
    ...
    common_geometry_object = CylindricalIntersection(radius=3, length=20)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("CylindricalIntersection.stp")
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='cylindrical_intersection.h5m',
        material_tags=['1'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='cylindrical_intersection.h5m',
        materials=[mat1]
    )
