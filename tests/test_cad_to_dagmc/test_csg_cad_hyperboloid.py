...
def test_compare(kwargs):
    ...
    common_geometry_object = Hyperboloid(waist_radius=3, top_radius=5, height=10)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("hyperboloid.stp")
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='hyperboloid.h5m',
        material_tags=['1'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='hyperboloid.h5m',
        materials=[mat1]
    )
