...
def test_compare(kwargs):
    ...
    common_geometry_object = ThinGap(box_width=10, box_height=10, box_depth=10, gap=0.5)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("ThinGap.stp")
    ...
    my_source.space = openmc.stats.Point((-5.25, 0, 0))
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='thin_gap.h5m',
        material_tags=['1', '2'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='thin_gap.h5m',
        materials=[mat1, mat2]
    )
