...
def test_compare(kwargs):
    ...
    common_geometry_object = SphereWithCylindricalHole(sphere_radius=10, cylinder_radius=2)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("sphere_with_cylindrical_hole.stp")
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='sphere_with_cylindrical_hole.h5m',
        material_tags=['1'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='sphere_with_cylindrical_hole.h5m',
        materials=[mat1]
    )
