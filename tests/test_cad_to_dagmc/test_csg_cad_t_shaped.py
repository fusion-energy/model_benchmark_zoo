from model_benchmark_zoo import TShaped
...
def test_compare(kwargs):
    ...
    common_geometry_object = TShaped(length=10, width=10, leg_thickness=3, height=5)
    # just writing a CAD step file for visulisation
    common_geometry_object.export_stp_file("t_shaped.stp")
    ...
    # Create a DT point source inside the stem of the T
    ...
    common_geometry_object.export_h5m_file_with_cad_to_dagmc(
        filename='t_shaped.h5m',
        material_tags=['1'],
        **kwargs
    )
    dag_model = common_geometry_object.dagmc_model(
        h5m_filename='t_shaped.h5m',
        materials=[mat1]
    )
